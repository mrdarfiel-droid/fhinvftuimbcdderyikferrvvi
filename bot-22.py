...
CHANGELOG_HISTORY = [
    ("5.4", "обновление", [
        ...
        "➕ Репутация — без точки: просто ответь на сообщение «+» или «респект» (а «-» или «дизреспект» — понизить). Старые команды с точкой тоже работают.",
        "⏰ Дневной отчёт владельцу уходит точно в 23:59 МСК по расписанию. «Первый писавший» теперь переживает перезапуск бота — дублей не будет.",
        "📊 В отчёте появилась экономика дня: сколько LP выпущено и сожжено, и темп — на сколько пассов в месяц идёт клан.",
        "🚫 Сообщения-команды (типа «.help») больше не считаются активностью и не приносят LP.",
        "🎬 Гифки приветствий стали надёжнее: бот берёт лёгкий формат, который Telegram доставляет всегда.",
        "⚖️ Баланс: новички стартуют с 10 LP (было 50); владелец скрыт из всех публичных топов — борьба за №1 стала настоящей.",
    ]),
    ("5.3", "обновление", [
