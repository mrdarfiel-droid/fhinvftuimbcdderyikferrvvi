...
QUIZ_REWARD = 15              # 5.8: банк за победу в викторине (LP). Снижено 25→15
...
def init_db():
    ...
    _add_col("users", "earned_title", "TEXT")             # 5.9: престижный титул, заработанный за квесты
...
def prank_menu_text():
    return (f"{pe('game')} <b>ПРАНК-МЕНЮ ВЛАДЕЛЬЦА</b>\n{CLAN_LINE}\n"
            ...
            f"({PRANK_COUNTDOWN_SEC} сек) и разоблачением в конце.\n\n"
            f"<i>Реальная экономика, баны и муты НЕ затрагиваются — это чистый розыгрыш.</i>")


async def _run_countdown_prank(context, code):
    """5.9: запускает фейк-пранк с обратным отсчётом в чате клана."""
    ...
    try:
        _btn, buildup, reveal = sc[0], sc[1], sc[2]
        try:
        ...
        try:
            await context.bot.edit_message_text(reveal, chat_id=cid, message_id=msg.message_id, parse_mode=ParseMode.HTML)
        except Exception:
            try:
                await context.bot.send_message(cid, reveal, parse_mode=ParseMode.HTML)
            except Exception as e:
                log_error("пранк-финал", e)
    finally:
...
async def handle_triggers(update, context, text, low, parts):
        ...
        while i < len(toks):
            ...
            if key in _PRANK_FIELDS and i + 1 < len(toks):
                ...
                if otype == "int":
                    ...
                    overlay[okey] = clean_title(" ".join(words)) if okey in ("title",) else " ".join(words)[:40]
                    applied.append(f"{key}=«{overlay[okey]}»")
                    i = j
            else:
                i += 1
    ...
    if first in ("викторина", "quiz", "квиз"):
        ...
        await update.message.reply_text(
            ...
            f"{esc(q['q'])}\n\n{opts_txt}\n\n"
            f"{pe('lp')} Банк: <b>{QUIZ_REWARD} LP</b> · ответь номером: <code>.answer N</code> · "
    ...
    if first in ("ответ", "answer", "otvet"):
        ...
        if int(pick) == st["answer"]:
            ...
            if me.id not in EXCLUDED_IDS and me.id != LEADER_ID:
                given, _ = award_lp_capped(cid, me.id, QUIZ_REWARD)
            await update.message.reply_text(
                f"{pe('cup')} <b>{esc(me.full_name)}</b> отвечает верно и забирает банк — <b>+{int(given)} LP</b>! {pe('fire')}",
                parse_mode=ParseMode.HTML)
        else:
            await update.message.reply_text(
...
PRANK_SCENARIOS = {
    "ban": ("💣 Фейк-бан всех",
            ...
            f"Запускаю <b>АВТОБАН ВСЕХ участников</b>. Отмена невозможна.",
            f"{pe('catlol') if False else '😜'} <b>ШУТКА!</b> Никто не забанен 😄\n"
            f"Это был розыгрыш от администрации. Всем мира и актива! 💙"),
    "mute": ("🔇 Фейк-мут чата",
             f"{pe('mute')} <b>СИСТЕМА:</b> за флуд активируется <b>тотальный мут чата на 24 часа</b>.\n"
             f"Применяется ко всем без исключения.",
             f"😄 <b>Расслабьтесь — это прикол!</b> Чат свободен, пишите дальше 💬"),
    "wipe": ("🗑 Фейк-сброс LP",
             f"{pe('warn')} <b>ВНИМАНИЕ:</b> техническая ошибка — запущен <b>сброс ВСЕХ балансов LP до нуля</b>.\n"
             f"Пытаюсь отменить…",
             f"😅 <b>Фух, успели отменить!</b> Ваши LP на месте. Шутка от админа 😎"),
    "raid": ("🚨 Фейк-тревога рейда",
             f"{pe('warn')} <b>ТРЕВОГА!</b> Обнаружен рейд ботов. Чат <b>блокируется для защиты</b>.\n"
             f"Вход и сообщения будут закрыты через…",
             f"😆 <b>Ложная тревога!</b> Это был розыгрыш. Всё под контролем 🛡️"),
    "gift": ("🎁 Фейк-раздача LP",
             f"{pe('lp')} <b>СЕКРЕТНАЯ АКЦИЯ!</b> Всем участникам — <b>по 1000 LP</b> за активность!\n"
             f"Начисление через…",
             f"😈 <b>Купились?</b> 1000 LP не будет… но за реакцию спасибо! Шутка 💙"),
    "kick": ("👢 Фейк-чистка неактива",
             f"{pe('ban')} <b>АВТО-ЧИСТКА:</b> удаляю всех, кто писал мало за неделю.\n"
             f"Список сформирован, исключение через…",
             f"😜 <b>Пронесло!</b> Никого не выгнали. Но актив подтяните 😏 Шутка!"),
}
...
def active_title(row):
    """Действующий титул под именем. Приоритет: купленный/временный (если не истёк) → заработанный за квесты (5.9)."""
    try:
        ...
        if t and not (until and until < today_str()):
            return t
        # 5.9: иначе показываем престижный титул, заработанный квестами
        et = row["earned_title"] if "earned_title" in row.keys() else None
        return et or ""
    except Exception:
        return ""
...
def update_quest_title(chat_id, user_id):
    """5.9: выставляет престижный титул по числу выполненных квестов. Возвращает (новый_титул|None, всего)."""
    total = quests_total(chat_id, user_id)
    best, reached_new = None, None
...
def _help_section(key):
    ...
    if key == "profile":
        return (f"{pe('person')} <b>Профиль и статистика</b>\n<blockquote expandable>"
                ...
                "• <code>.title buy ТЕКСТ</code> — личный титул в профиле\n"
