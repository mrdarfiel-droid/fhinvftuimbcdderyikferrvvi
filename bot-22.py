...
QUEST_REWARD = 10              # 5.8: награда за квест дня (LP). Снижено 15→10 (идёт через общий недельный лимит)
...
CHANGELOG_HISTORY = [
    ("5.8", "Арсенал", [
        "🎯 Квесты дня: у каждого своё задание (написать N сообщений, получить респект, сыграть дуэль…) за 15 LP. Смотри своё: <code>.квест</code>. Бот сам замечает выполнение и поздравляет.",
        "🏅 Значки-ачивки: за серии, активность, ранг, репутацию и победы. Постоянные награды в профиле — смотри <code>.ачивки</code>. Открываются автоматически.",
        "❓ Викторина по Brawl Stars: <code>.викторина</code> запускает вопрос, первый верный <code>.ответ N</code> забирает банк 25 LP.",
        "📅 Нормы кубков: проверка по-прежнему еженедельная, но LP теперь начисляются раз в месяц «большой проверкой» 1-го числа — +70 LP за каждую закрытую неделю. Отыграть нормы только в конце месяца не выйдет: считается каждая неделя отдельно. Штрафы и норм-варны остаются еженедельными.",
        "✨ Профиль теперь показывает значки игрока в ряд под рангом.",
        "⚖️ Экономика поджата ещё раз: LP за сообщение 0.03→0.02, дневной потолок 700→600 (теперь ~12 LP/день). Награды квеста (10) и викторины (15) идут через общий недельный лимит вместе с нормами — не суммируются сверх 150 LP/нед.",
    ]),
    ("5.7", "Казна", [
        "🎰 Лотерея: купи билет за 20 LP (<code>.лотерея билет</code>), банк растёт, администрация проводит розыгрыш — победитель забирает всё.",
        "⚔️ Дуэли на LP: ответь сопернику <code>.дуэль 50</code>, он принимает — победитель (50/50) забирает ставку. Ставка 5–50 LP, комиссия казны 10%, лимиты от слива баланса.",
        "⭐ Титулы: свой титул в профиле на 7 дней за 100 LP (<code>.титул купить ТЕКСТ</code>); администрация может выдавать титулы вручную.",
        "🎫 Лимит магазина: 1 пасс на человека в календарный месяц — защита бюджета клана.",
        "💰 Экономика занёрфлена: LP за сообщение 0.05 → 0.03, дневной потолок 1500 → 700 сообщений (до 21 LP/день вместо 75), серии и потолок наград за нормы снижены.",
        "🧹 Все штабные команды (бан, мут, варн, lp, отчёт, тег…) сами удаляются из чата после вызова — чат остаётся чистым.",
        "📜 <code>.docs</code> стал персональным (листает только открывший); починены кнопки топа на английском (top today).",
        "🏆 Автоподсчёт кубков доведён: ежедневный скан в 13:00 сам обновляет кубки и ставит недельную точку отсчёта — тег достаточно привязать один раз.",
    ]),
    ("5.6", "обновление", [
        "🏅 Ранги Brawl Stars — ОФИЦИАЛЬНО: проверено боем, ранги совпадают с игрой. «ранг» — твой ранг, «топ ранг» — топ клана, ранг виден и в профиле.",
        ...
        "🔭 Ежедневно в 13:00 бот сам обновляет кубки, ники и ранги всех привязанных — «обновлять тег» вручную больше не нужно.",
        "🛠 КРИТФИКС кубков: точку отсчёта недели больше невозможно сбросить случайно — все снимки (включая владельческие) защищены и требуют слово «заново»; перепривязка тега базу не трогает, а привязка среди недели честно начинает отсчёт с этого момента.",
        "🔬 Диагностика: «нормы» — разбивка по дням и «было → сейчас» по кубкам; «актив» (модераторам) — сырые записи счётчика; сбои счётчика — в журнал ошибок.",
        "🧪 Для админов: «нормы тест» и «нормы снимок» — обкатка норм до 22 июня; «ранг тест» — сверка карты рангов по живым боям.",
        "➕ Полировка: понятные имена (@ник/ID) в нормах, у проваливших видна и выполненная часть, исправлено «++0» в «lp», ноль LP вежливо отклоняется.",
    ]),
    ("5.5", "обновление", [
...
async def show_profile(update, context, user):
    ...
    text = (
        ...
        f"{esc(user.full_name)}{uname}{title_line}\n\n"
...
async def handle_triggers(update, context, text, low, parts):
    ...
    if first in ("квест", "quest", "задание"):
        ...
        if done:
            await update.message.reply_text(
                ...
                f"{pe('lp')} Награда получена. Новый квест — завтра в 00:00 МСК.",
                parse_mode=ParseMode.HTML)
            return True
        ...
        await update.message.reply_text(
            ...
            f"{pe('lp')} Награда: <b>{QUEST_REWARD} LP</b> · выполни до конца дня (МСК)",
            parse_mode=ParseMode.HTML)
        return True
    ...
    if first in ("викторина", "quiz", "квиз"):
        ...
        await update.message.reply_text(
            ...
            f"{esc(q['q'])}\n\n{opts_txt}\n\n"
            f"{pe('lp')} Банк: <b>{QUIZ_REWARD} LP</b> · ответь номером: <code>.ответ N</code> · "
    ...
    if first in ("ответ", "answer", "otvet"):
        ...
        if not st:
            await update.message.reply_text(f"{pe('note')} Сейчас нет активной викторины. Запусти: <code>.викторина</code>", parse_mode=ParseMode.HTML)
            return True
        ...
        if not pick:
            await update.message.reply_text(f"{pe('note')} Ответь номером: <code>.ответ 2</code>", parse_mode=ParseMode.HTML)
            return True
        me = update.effective_user
    ...
    if first in ("лотерея", "лото", "lottery"):
        ...
        if sub in ("статус", "info", ""):
            ...
            await update.message.reply_text(
                ...
                f"{'✅ Ты в игре!' if mine else 'Купить билет: <code>.лотерея билет</code>'}\n"
    ...
    if first in ("дуэль", "duel", "дуель"):
        ...
        if not update.message.reply_to_message or not bet_arg:
            await update.message.reply_text(
                ...
                f"Ответь на сообщение соперника: <code>.дуэль {DUEL_MIN}–{DUEL_MAX}</code>\n"
                f"Он принимает: <code>.дуэль принять</code>. Победитель забирает ставку.\n"
        ...
        await update.message.reply_text(
            ...
            f"{mention(target)}, принимаешь? <code>.дуэль принять</code> "
            f"(или <code>.дуэль отмена</code>). Время: {DUEL_TIMEOUT_MIN} мин.",
            parse_mode=ParseMode.HTML)
        return True
    ...
    if first in ("титул", "title"):
        ...
        if sub in ("купить", "buy"):
            ...
            if not wanted:
                await update.message.reply_text(
                    ...
                    f"Команда: <code>.титул купить ТВОЙ ТЕКСТ</code> (до {TITLE_MAX_LEN} симв.)",
                    parse_mode=ParseMode.HTML)
                return True
        ...
        await update.message.reply_text(
            ...
            if cur else
            f"{pe('game')} У тебя нет титула. Купить на {TITLE_TEMP_DAYS} дней: <code>.титул купить ТЕКСТ</code> ({TITLE_TEMP_COST} LP).",
            parse_mode=ParseMode.HTML)
        return True
...
def active_title(row):
    """5.7: действующий титул участника ('' если нет/просрочен)."""
    try:
...
async def try_complete_quest(context, chat_id, user_id, name):
    """Проверяет квест дня и выдаёт награду, если выполнен и ещё не получен. Тихо, без спама."""
    ...
    conn.commit()
    given, _bal = award_lp_capped(chat_id, user_id, QUEST_REWARD)
    if given > 0 and ALLOWED_CHAT_ID:
        try:
            await context.bot.send_message(
                ALLOWED_CHAT_ID,
                f"{pe('check')} <b>{esc(name)}</b> выполнил квест дня «{desc}» — <b>+{int(given)} LP</b>! {pe('fire')}",
                parse_mode=ParseMode.HTML)
        except Exception as e:
...
def duel_block_reason(chat_id, a, b):
    ...
    Защита от слива баланса другу через поддавки."""
    today = today_str()
...
def _help_section(key):
    ...
    if key == "profile":
        return (f"{pe('person')} <b>Профиль и статистика</b>\n<blockquote expandable>"
                ...
                "• <code>.ранг</code> — твой ранг (за новые ступени — LP!) · <code>.топ ранг</code> — топ клана\n"
                "• <code>.лотерея</code> — купить билет и выиграть банк · <code>.дуэль</code> (ответом) — сразиться на LP\n"
                "• <code>.титул купить ТЕКСТ</code> — личный титул в профиле\n"
                "• <code>.квест</code> — задание дня за LP · <code>.ачивки</code> — твои значки · <code>.викторина</code> — вопрос с банком LP\n"
    ...
    if key == "admin":
        return (f"{pe('shield')} <b>Админ-команды</b> <i>(работают только у администрации)</i>\n<blockquote expandable>"
                ...
                "<code>.лотерея розыгрыш</code> — провести розыгрыш лотереи · <code>.титул дать @кто ТЕКСТ</code> — выдать титул\n"
