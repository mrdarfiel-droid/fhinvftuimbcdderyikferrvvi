...
async def maybe_daily_report(context, chat_id):
    """Раз в сутки шлёт владельцу отчёт за ПРОШЕДШИЙ день. Срабатывает фоном с первым
    сообщением нового дня (планировщика на хостинге нет, поэтому по событиям, как и нормы)."""
    if not ALLOWED_CHAT_ID or chat_id != ALLOWED_CHAT_ID:
        return
    yesterday = (msk_now() - timedelta(days=1)).strftime("%Y-%m-%d")
    if meta_get("report_done_for") == yesterday:
        return
    meta_set("report_done_for", yesterday)   # ставим сразу, чтобы отчёт не задвоился
    asyncio.create_task(send_daily_report(context, chat_id, yesterday))
