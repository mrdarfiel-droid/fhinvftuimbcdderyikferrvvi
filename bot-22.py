...
BOT_VERSION = "5.0"
PATCH_CODE = "LB-5.0"
PATCH_NAME = "Цитадель"        # глобальный патч 5.0 «Цитадель» (раскатывается по этапам)
PATCH_TYPE = "глобальное обновление"
...
def brawl_profile_line(chat_id, user_id):
    """Строка с тегом для профиля (только кэш, без сети)."""
    row = get_user(chat_id, user_id)
    ...
    if data:
        return (f"\n\n<blockquote>{pe('game')} <b>Brawl Stars</b> · #{esc(tag)}\n"
                f"{pe('cup')} Кубки: <b>{fmt_num(data.get('trophies', 0))}</b> · "
                f"рекорд <b>{fmt_num(data.get('highestTrophies', 0))}</b></blockquote>")
    return f"\n\n<blockquote>{pe('game')} Brawl Stars: #{esc(tag)} — напиши «tag», чтобы обновить кубки.</blockquote>"
...
def brawl_card(data, tag, bound=False):
    ...
    return (f"{head}\n{CLAN_LINE}\n"
            ...
            f"{pe('shield')} Клуб: <b>{club_name}</b></blockquote>\n"
            f"<i>Следи за нормой клана: прирост +1450 кубков в неделю.</i>")
...
async def act_mute(update, context, t, minutes, reason):
    log_action(update, t, f"мут {minutes} мин", reason)
    await do_mute(context, update.effective_chat.id, t.id, minutes)
    when = f"{minutes} мин" if minutes < 60 else f"{round(minutes/60,1)} ч"
    await update.message.reply_text(
        f"{pe('mute')} <b>{esc(t.full_name)}</b> получил мут на {when}.\nПричина: {esc(reason)}", parse_mode=ParseMode.HTML)
...
async def show_profile(update, context, user):
    ...
    text = (
        ...
        f"{pe('clock')} В клане с: <b>{fmt_date(row['joined'])}</b>\n"
...
async def give_rep(update, context, t):
    chat_id = update.effective_chat.id
    giver = update.effective_user
    if t.id == giver.id:
        return await update.message.reply_text("Нельзя поднимать репутацию самому себе 🙂")
        ...
        try:
            ...
            if msk_now() - last < timedelta(minutes=REP_COOLDOWN_MIN):
                ...
                return await update.message.reply_text(
                    f"{pe('clock')} Репутацию можно давать раз в {REP_COOLDOWN_MIN} мин. Подожди ещё {left} мин.",
                    parse_mode=ParseMode.HTML)
        except Exception:
            pass
    ensure_user(chat_id, t)
    conn.execute("UPDATE users SET reputation=reputation+1 WHERE chat_id=? AND user_id=?", (chat_id, t.id))
    conn.execute("UPDATE users SET last_rep=? WHERE chat_id=? AND user_id=?", (msk_now().isoformat(), chat_id, giver.id))
    conn.commit()
    await update.message.reply_text(f"⭐ <b>{esc(t.full_name)}</b> получает +1 к репутации!", parse_mode=ParseMode.HTML)
...
async def show_hall_of_fame(update, context):
    ...
    if not rows:
        return await update.message.reply_text(
            f"{pe('cup')} Зал славы пока пуст — первые победители появятся в конце сезона.", parse_mode=ParseMode.HTML)
    medals = {1: pe("gold"), 2: pe("silver"), 3: pe("bronze")}
    seasons = {}
    for r in rows:
        seasons.setdefault(r["season"], []).append(r)
    blocks = []
    for season, items in list(seasons.items())[:6]:
        winners = " · ".join(f"{medals.get(it['rank'], '')} {esc(it['name'])}"
...
def main_menu_keyboard():
    """Главное меню (инлайн-кнопки). В подписях кнопок только обычные эмодзи (премиум там не работают)."""
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("👤 Профиль", callback_data="menu:profile"),
         InlineKeyboardButton("🛒 Магазин", callback_data="menu:shop")],
        [InlineKeyboardButton("🏆 Топы", callback_data="menu:top"),
         InlineKeyboardButton("📅 Сезон", callback_data="menu:season")],
        [InlineKeyboardButton("🔥 Стрик", callback_data="menu:streak"),
         InlineKeyboardButton("👑 Зал славы", callback_data="menu:hof")],
        [InlineKeyboardButton("📋 Нормы", callback_data="menu:norms"),
...
async def docs_info(update, context):
    ...
    await update.message.reply_text(
        ...
        parse_mode=ParseMode.HTML)
    # История изменений — отдельными сообщениями, чтобы не упереться в лимит Telegram (4096).
    ...
    for ver, kind, items in CHANGELOG_HISTORY:
        ...
        block = premiumize(f"{pe('star')} <b>{vlabel} — {kind}</b>\n" + "\n".join(items))
...
async def on_action_callback(update, context):
    ...
    if data.startswith("faq:"):
        ...
        if choice == "yes":
            ...
            try:
                ...
                pass
        else:  # отказ — вариант А: объясняем, выбор за участником
            await cq.answer()
            try:
                await cq.edit_message_text(
    ...
    if data.startswith("menu:"):
        ...
        pu = _proxy_update(update)
        if action == "profile":
            await show_profile(pu, context, user)
        elif action == "shop":
            await store_info(pu, context)
        elif action == "top":
            await show_top(pu, context, ["active"])
        elif action == "season":
            await show_season(pu, context)
        elif action == "hof":
            await show_hall_of_fame(pu, context)
        elif action == "norms":
            await norms_info(pu, context)
        elif action == "rules":
            await rules_cmd(pu, context)
        elif action == "streak":
            row = get_user(pu.effective_chat.id, user.id)
            cur = (row["streak"] or 0) if row else 0
            best = (row["streak_best"] or 0) if row else 0
            await cq.message.reply_text(
                f"{pe('fire')} Твоя серия: <b>{cur} {_days_word(cur)} подряд</b>\n"
                f"{pe('star')} Рекорд: <b>{best} {_days_word(best)}</b>", parse_mode=ParseMode.HTML)
        elif action == "help":
            await cq.message.reply_text(_help_member_text(), parse_mode=ParseMode.HTML)
        return
...
async def ai_fun(update, context, kind):
    ...
    answer = await ai_complete(sys_prompt, user_prompt, temperature=1.05)
...
async def start_cmd(update, context):
    ...
    # если не принял соглашение (или пришёл по кнопке «принять») — показываем соглашение с кнопками
    if user and (arg == "agree" or not has_agreed(user.id)):
    ...
    await update.message.reply_text(
        ...
        f"В личке доступно: <b>profile</b> · <b>appeal</b> · <b>faq</b> · <b>help</b>", parse_mode=ParseMode.HTML)
