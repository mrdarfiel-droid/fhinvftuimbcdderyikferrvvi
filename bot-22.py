...
QUIZ_TIMEOUT_SEC = 90        # 6.0: время на ответ кнопкой в викторине 2.0
QUIZ_DIFFICULTY = {1: ("🟢", "Лёгкий"), 2: ("🟡", "Средний"), 3: ("🟠", "Сложный"), 4: ("🔴", "Эксперт")}


def quiz_catline(diff, cat):
...
# ===== 6.0 «ИМПЕРИЯ»: ивент-модуль. Теперь ивенты РАЗНЫЕ (не только LP). kind: lp/rep/quiz =====
# имя -> (подпись, множитель, тип)
EVENT_TYPES = {
    "double":  ("🔥 Двойные LP", 2.0, "lp"),
    "triple":  ("⚡ Тройные LP", 3.0, "lp"),
    "happy":   ("🎉 Счастливый час (x2 LP)", 2.0, "lp"),
    "rep":     ("💛 Час репутации (x2 респекты)", 2.0, "rep"),
    "quiz":    ("🧠 Викторина-марафон (без задержки)", 1.0, "quiz"),
}
EVENT_KIND_BY_NAME = {label: kind for (label, _m, kind) in EVENT_TYPES.values()}
EVENT_MAX_HOURS = 72            # максимальная длительность ивента (часы)


def event_active():
    """Возвращает (название, множитель, тип) активного ивента или (None, 1.0, None)."""
    ...
    try:
        ...
        mult = 1.0
    kind = meta_get("event_kind") or EVENT_KIND_BY_NAME.get(name, "lp")
    return name, mult, kind


def event_multiplier():
    """Множитель LP за сообщения (1.0, если ивент не LP-типа или его нет)."""
    name, mult, kind = event_active()
    return mult if kind == "lp" else 1.0


def rep_multiplier():
    """6.0: множитель репутации за респект (2x во время «Часа репутации»)."""
    name, mult, kind = event_active()
    return mult if kind == "rep" else 1.0


def quiz_rush_active():
    """6.0: True во время ивента «Викторина-марафон» — викторина без задержки."""
    return event_active()[2] == "quiz"
...
def event_status_text():
    ...
    try:
        ...
        left_s = "—"
    if kind == "rep":
        effect = f"{pe('star')} Респекты дают <b>x{mult:g}</b> репутации"
    elif kind == "quiz":
        effect = f"{pe('game')} Викторина работает <b>без задержки</b>"
    else:
        effect = f"{pe('lp')} Множитель LP за сообщения: <b>x{mult:g}</b>"
...
# 6.0: «скрытные» команды — фидбек уходит ТОЛЬКО в ЛС владельцу, в чате команда просто удаляется.
# Так участники не видят изменения баланса/репутации/пранки и не поднимают панику.
# Бан/мут/варн/кик сюда НЕ входят — они должны быть видны в чате (публичная модерация).
...
COMMANDS_TOPIC_ID = 73265      # 6.0: топик для бот-команд участников. Вне его (в основном чате) команды НЕ работают.
...
BOT_VERSION = "6.0"
PATCH_CODE = "LB-6.0"
UPDATE_NAME = "Империя"         # название большого обновления (ветка 6.x — финал)
...
CHANGELOG_HISTORY = [
    ("6.0", "Империя", [
        ...
        "🎁 <b>Розыгрыши LP!</b> Администрация может разыграть LP среди активных участников — чем активнее ты в чате, тем выше шанс попасть в число счастливчиков. Будь на связи!",
        "📣 Достижения теперь видны всем: бот объявляет в чате, когда кто-то <b>дорастает до нового кланового звания</b>. Расти и попади в ленту клана!",
        "🛒 Магазин обновлён: за LP теперь покупаются внутриклановые ценности (звания копятся сами, право на ивент, свой титул). Brawl Pass и гемы — за рубли у менеджера.",
        ...
        "🧠 Викторина 2.0: 40 вопросов по Brawl Stars (бойцы, режимы, механика) с 4 уровнями сложности — 🟢 Лёгкий, 🟡 Средний, 🟠 Сложный, 🔴 Эксперт. Категория и сложность видны прямо в вопросе. Без повторов, ответ КНОПКАМИ, одна попытка. Запусти: <code>.quiz</code>.",
    ...
    ("5.1", "обновление", [
        ...
        "🏆 Нормы запущены! Кубки и активность считаются по неделям (Пн→Вс). Закрывай нормы — получай LP и поднимайся в клановом звании. На старте — мягкий период без штрафов, чтобы все привыкли.",
    ...
    ("5.0", "глобальное обновление", [
        ...
        "🗓 Нормы кубков и активности — запущены с мягким стартом, чтобы все спокойно вошли в ритм.",
...
REACTION_PLUS = {"+", "+rep", "+реп", "респект", "respect", "красавчик", "красава", "краш", "молодец", "красавчик)"}
...
CLAN_TAG = os.environ.get("CLAN_TAG", "2U80CYP9J").upper().lstrip("#")   # 6.0: тег клана Linkor Empire (#2U80CYP9J) для сверки состава; можно переопределить через env
...
def init_db():
    ...
    _add_col("users", "rank_paid_month", "TEXT")           # 6.0: месяц (YYYY-MM), в котором считается лучший ранг для награды
    ...
    _add_col("users", "quiz_seen", "TEXT")                 # 6.0: показанные вопросы викторины (без повторов)
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS norm_warns (
        chat_id INTEGER, user_id INTEGER, ts TEXT)""")
    # 6.0: одноразовая миграция — у кого накопленный LP ещё не считался, ставим = текущему балансу
    conn.execute("UPDATE users SET lp_earned=lp WHERE COALESCE(lp_earned,0)=0 AND COALESCE(lp,0)>0")
    conn.commit()
    load_seed()
    load_seed_by_id()
    load_roles_by_id()
...
def add_lp(chat_id, user_id, delta):
    ...
    if applied != 0:
        ...
        # 6.0: накопленный LP (для клановых званий) растёт только на положительных начислениях
        if applied > 0:
            old_earned = conn.execute("SELECT COALESCE(lp_earned,0) AS e FROM users WHERE chat_id=? AND user_id=?",
                                      (chat_id, user_id)).fetchone()["e"]
            new_earned = old_earned + applied
            conn.execute("UPDATE users SET lp_earned=? WHERE chat_id=? AND user_id=?",
                         (new_earned, chat_id, user_id))
            # повышение звания? ставим в очередь на публичный анонс
            old_idx = clan_rank_for(old_earned)[2]
            new_idx = clan_rank_for(new_earned)[2]
            if new_idx > old_idx and user_id not in EXCLUDED_IDS:
                _pending_rankups.append((chat_id, user_id, CLAN_RANKS[new_idx][1]))
        conn.commit()
    return new


_pending_rankups = []   # 6.0: очередь повышений в звании для публичного анонса (chat_id, user_id, новое_звание)


async def flush_rankup_announcements(context):
    """6.0: публично объявляет повышения в клановом звании в основном чате (мотивация + движ)."""
    if not _pending_rankups:
        return
    items = _pending_rankups[:]
    _pending_rankups.clear()
    seen = set()
    for chat_id, user_id, rank_name_str in items:
        key = (chat_id, user_id, rank_name_str)
        if key in seen:
            continue
        seen.add(key)
        try:
            urow = get_user(chat_id, user_id)
            nm = (urow["name"] if urow else None) or "Боец"
            await context.bot.send_message(
                ALLOWED_CHAT_ID or chat_id,
                f"{pe('crown')} <b>НОВОЕ ЗВАНИЕ!</b>\n{CLAN_LINE}\n"
                f"{pe('fire')} <b>{esc(nm)}</b> дорос до звания <b>{rank_name_str}</b>! Поздравляем, боец 🎉\n"
                f"<i>Свой прогресс смотри командой .p в топике команд.</i>",
                parse_mode=ParseMode.HTML)
        except Exception as e:
            log_error("анонс звания", e)
...
def active_members(chat_id, days=7, min_msgs=1):
    """6.0: участники, писавшие в чат за последние `days` дней (для розыгрыша). Без вышедших и админов-вне-экономики."""
    since = (msk_now() - timedelta(days=days)).strftime("%Y-%m-%d")
    rows = conn.execute(
        "SELECT s.user_id, COALESCE(u.name,'боец') AS name, SUM(s.cnt) AS msgs "
        "FROM stats s LEFT JOIN users u ON u.chat_id=s.chat_id AND u.user_id=s.user_id "
        "WHERE s.chat_id=? AND s.day>=? AND COALESCE(u.gone,0)=0 "
        "GROUP BY s.user_id HAVING msgs>=? ", (chat_id, since, min_msgs)).fetchall()
    return [r for r in rows if r["user_id"] not in EXCLUDED_IDS]
...
# ============================================================
# 6.0 КЛАНОВЫЕ ЗВАНИЯ — престиж по НАКОПЛЕННОМУ за всё время LP (трата LP их НЕ понижает).
# Это главный ценный статус участника: виден в профиле и растёт за активность.
...
def clan_rank_for(earned):
    """6.0: (название, описание, индекс) звания по накопленному LP."""
    earned = earned or 0
    idx = 0
    for i, (need, _name, _desc) in enumerate(CLAN_RANKS):
        if earned >= need:
            idx = i
    return CLAN_RANKS[idx][1], CLAN_RANKS[idx][2], idx


def clan_rank_next(earned):
    """6.0: (название_следующего, сколько_LP_осталось) или (None, 0) если максимум."""
    earned = earned or 0
    for need, name, _desc in CLAN_RANKS:
        if earned < need:
            return name, need - earned
    return None, 0


def user_lp_earned(chat_id, user_id):
    row = conn.execute("SELECT COALESCE(lp_earned,0) AS e FROM users WHERE chat_id=? AND user_id=?",
                       (chat_id, user_id)).fetchone()
...
RUBLE_ITEMS = ["Brawl Pass", "Brawl Pass Plus", "Pro Pass", "гемы"]
...
def progress_line(lp, earned=None):
    """6.0: прогресс до следующего кланового ЗВАНИЯ (по накопленному LP). Звания — главная цель прогресса."""
    earned = (earned if earned is not None else lp) or 0
    cur_name, _desc, _idx = clan_rank_for(earned)
    nxt_name, left = clan_rank_next(earned)
    if not nxt_name:
        return f"{pe('crown')} Звание: <b>{cur_name}</b> — максимум! Ты легенда клана 🔥"
    need = earned + left
    pct = int(min(100, earned / need * 100)) if need else 100
    return (f"{pe('crown')} Звание: <b>{cur_name}</b>\n"
            f"{pe('star')} До «{nxt_name}»: <b>{fmt_lp(left)}</b> LP (накоплено всего: {fmt_lp(earned)})\n"
            f"{progress_bar(earned, need)} {pct}%")
...
RANK_TIER_REWARDS = {3: 20, 4: 40, 5: 70, 6: 100, 7: 150}   # Алмаз/Мифик/Легенда/Мастер/Про — LP
...
def apply_rank_progress(chat_id, user_id, value):
    """6.0: награда выдаётся ТОЛЬКО за ЛУЧШИЙ ранг в текущем месяце (не за каждую ступень).
    Каждый месяц прогресс наград обнуляется. Возвращает (лучшее_значение, [(ступень, lp)])."""
    row = conn.execute(
        "SELECT rank_best, COALESCE(rank_paid_tier,-1) AS paid, rank_paid_month FROM users WHERE chat_id=? AND user_id=?",
        (chat_id, user_id)).fetchone()
    if not row:
        return None, []
    month = today_str()[:7]
    # лучший «сырой» ранг за всё время (для профиля/топов) обновляем как раньше
    best = row["rank_best"]
    if best is None or value > best:
        best = value
        conn.execute("UPDATE users SET rank_best=?, rank_seen=? WHERE chat_id=? AND user_id=?",
                     (best, today_str(), chat_id, user_id))
    # награда считается по ТЕКУЩЕМУ рангу (value), в рамках месяца
    cur_tier = rank_tier(value)
    paid_tier = row["paid"]
    # новый месяц → сбрасываем оплаченную ступень
    if row["rank_paid_month"] != month:
        paid_tier = -1
        conn.execute("UPDATE users SET rank_paid_month=?, rank_paid_tier=-1 WHERE chat_id=? AND user_id=?",
                     (month, chat_id, user_id))
    awards = []
    if cur_tier in RANK_TIER_REWARDS and cur_tier > paid_tier:
        # награда за лучший ранг месяца: ценность текущей ступени минус уже оплаченной
        # (плоские награды ступеней: Алмаз 20, Мифик 40, Легенда 70, Мастер 100, Про 150)
        prev_val = RANK_TIER_REWARDS.get(paid_tier, 0) if paid_tier in RANK_TIER_REWARDS else 0
        reward = RANK_TIER_REWARDS[cur_tier] - prev_val
        if reward > 0:
            add_lp(chat_id, user_id, reward)
            awards.append((_RANK_TIERS[cur_tier], reward))
        conn.execute("UPDATE users SET rank_paid_tier=?, rank_paid_month=? WHERE chat_id=? AND user_id=?",
                     (cur_tier, month, chat_id, user_id))
    conn.commit()
    return best, awards
...
def _strip_emoji(s):
    """Убирает эмодзи и небуквенные символы (кроме базовой пунктуации) — на карте они дают квадрат."""
    return re.sub(r"[^\w\s\-!?.,'·]", "", (s or ""), flags=re.UNICODE).strip()


def _draw_star(d, cx, cy, r_out, r_in, fill, rot=-90):
    import math
    pts = []
    for k in range(10):
        ang = math.radians(k * 36 + rot)
...
def render_profile_card(chat_id, user_id, display_name):
    """6.0: карточка профиля в тёмно-золотом стиле клана (топ-дизайн). BytesIO PNG, None при сбое."""
    try:
        ...
        PX = 56 * SS                 # левый отступ контента
        # шапка: звезда + LINKOR
        ...
        if et_clean:
            d.text((PX + 2 * SS, 190 * SS), et_clean[:34], font=F(20, False), fill=gold_lt)

        # бейдж ранга (правый верх)
        rb = row["rank_best"] if "rank_best" in row.keys() else None
        if rb:
            rtext = _ct(rank_name(rb))
            bb = d.textbbox((0, 0), rtext, font=F(18, True))
            bw = (bb[2] - bb[0]) + 36 * SS
            bx2 = W - 40 * SS
            bx1 = bx2 - bw
            d.rounded_rectangle([bx1, 140 * SS, bx2, 184 * SS], radius=10 * SS, fill=gold)
            d.text((bx1 + 18 * SS, 148 * SS), rtext, font=F(18, True), fill=(20, 22, 30))
        ...
        colx = [PX, W // 2 + 20 * SS]
        data = [
            [("LP BALANCE", fmt_lp(lp)), ("REPUTATION", str(rep)), ("DAY STREAK", str(streak))],
            [("MESSAGES", fmt_num(total) if 'fmt_num' in globals() else str(total)),
             ("RANK", _ct(rank_name(rb)) if rb else "—"), ("QUESTS", str(quests_total(chat_id, user_id)))],
        ]
        for ci, col in enumerate(data):
            for ri, (label, val) in enumerate(col):
                yy = y0 + ri * rowh
                # маленький золотой маркер
                d.rectangle([colx[ci], yy + 4 * SS, colx[ci] + 4 * SS, yy + 18 * SS], fill=gold)
                d.text((colx[ci] + 16 * SS, yy), label, font=F(15, False), fill=gold)
                d.text((colx[ci] + 16 * SS, yy + 22 * SS), str(val), font=F(28, True), fill=white)
        # разделитель колонок
        d.line([W // 2, y0 - 6 * SS, W // 2, y0 + 3 * rowh - 10 * SS], fill=gold_dk, width=1 * SS)

        # подвал
        d.line([PX, H - 64 * SS, W - 40 * SS, H - 64 * SS], fill=gold_dk, width=1 * SS)
        d.text((PX, H - 50 * SS), "BRAWL STARS COMMUNITY", font=F(14, False), fill=grey)
        dt = today_str()
        db = d.textbbox((0, 0), dt, font=F(14, False))
        d.text((W - 40 * SS - (db[2] - db[0]), H - 50 * SS), dt, font=F(14, False), fill=grey)

        # даунсэмпл для гладкости
...
async def show_profile(update, context, user):
    ...
    if user.id not in EXCLUDED_IDS:
        text += f"\n\n<blockquote>{progress_line(row['lp'], row['lp_earned'] if 'lp_earned' in row.keys() else None)}</blockquote>"
...
async def give_rep(update, context, t, delta=1):
    ...
    eff_delta = delta
    if delta > 0:
        eff_delta = int(round(delta * rep_multiplier()))   # 6.0: x2 во время «Часа репутации»
    conn.execute("UPDATE users SET reputation=reputation+? WHERE chat_id=? AND user_id=?", (eff_delta, chat_id, t.id))
    ...
    conn.execute("INSERT INTO rep_log (chat_id, from_id, to_id, day, delta) VALUES (?,?,?,?,?)",
                 (chat_id, giver.id, t.id, today_str(), eff_delta))   # 5.8: для квеста «получи респект»
    conn.commit()
    row = get_user(chat_id, t.id)
    newrep = row["reputation"] if row else 0
    if delta >= 0:
        bonus = " <b>(x2 ивент!)</b>" if eff_delta > delta else ""
        await update.message.reply_text(
            f"{pe('star')} <b>{esc(t.full_name)}</b> получает +{eff_delta} к репутации!{bonus} Сейчас: <b>{newrep}</b>",
            parse_mode=ParseMode.HTML)
...
async def show_season(update, context):
    ...
    # прогресс сезона (сколько дней прошло из месяца)
    import calendar as _cal
    total_days = _cal.monthrange(msk_now().year, msk_now().month)[1]
    passed = max(0, total_days - left)
    head = (f"{pe('cup')} <b>СЕЗОН «{season_title(season)}» — {CLAN_NAME}</b>\n{CLAN_LINE}\n"
            f"{pe('clock')} До конца сезона: <b>{left} {_days_word(left)}</b>\n"
            f"{progress_bar(passed, total_days)} {int(passed/total_days*100)}%\n\n")
    if not rows:
        return await update.message.reply_text(
            head + "Пока никто не активничал в этом сезоне — будь первым! 🔥", parse_mode=ParseMode.HTML)
    medals = {0: pe("gold"), 1: pe("silver"), 2: pe("bronze")}
    lines = [f"{pe('growth')} <b>Топ активных в сезоне:</b>", "<blockquote>"]
    for i, r in enumerate(rows):
        marker = medals.get(i) or pe("star")
        lines.append(f"<b>{i+1}.</b> {marker} {esc(r['name'])} — <b>{r['val']}</b>")
    lines.append("</blockquote>")
    tail = (f"{pe('crown')} <b>Награды топ-3 в конце сезона:</b>\n"
            f"<blockquote>{pe('gold')} 1 место: +{SEASON_REWARDS[1]} LP · 🏆 титул «Чемпион сезона»\n"
            f"{pe('silver')} 2 место: +{SEASON_REWARDS[2]} LP · 🥈 «Призёр сезона»\n"
            f"{pe('bronze')} 3 место: +{SEASON_REWARDS[3]} LP · 🥉 «Призёр сезона»\n"
            f"Титул-регалия — в профиль на месяц + место в зале славы (<code>hof</code>).</blockquote>")
    await update.message.reply_text(head + "\n".join(lines) + "\n" + tail, parse_mode=ParseMode.HTML)


def _hof_leaders(chat_id, kind, limit=3):
    """Топ-N участников по категории для Зала Славы. 6.0: исключаем вышедших/удалённых (gone)."""
    if kind == "lp":
        return conn.execute("SELECT name, lp AS val FROM users WHERE chat_id=? AND COALESCE(gone,0)=0 ORDER BY val DESC LIMIT ?",
                            (chat_id, limit)).fetchall()
    if kind == "rep":
        return conn.execute("SELECT name, reputation AS val FROM users WHERE chat_id=? AND COALESCE(gone,0)=0 ORDER BY val DESC LIMIT ?",
                            (chat_id, limit)).fetchall()
    if kind == "trophies":
        return conn.execute(
            "SELECT COALESCE(brawl_name, name) AS name, brawl_trophies AS val FROM users "
            "WHERE chat_id=? AND COALESCE(gone,0)=0 AND brawl_tag IS NOT NULL AND brawl_trophies IS NOT NULL "
...
async def show_hall_of_fame(update, context):
    ...
    # победители сезонов (если есть) — 6.0: скрываем вышедших/удалённых из клана
    rows = conn.execute(
        "SELECT h.season, h.rank, h.name, h.score FROM hall_of_fame h "
        "LEFT JOIN users u ON u.chat_id=h.chat_id AND u.user_id=h.user_id "
        "WHERE h.chat_id=? AND COALESCE(u.gone,0)=0 ORDER BY h.season DESC, h.rank ASC",
        (chat_id,)).fetchall()
    season_block = ""
    if rows:
        seasons = {}
...
def _store_section(key):
    ...
    if key == "ranks":
        lines = [f"{pe('crown')} <b>Клановые звания</b>", "<blockquote expandable>",
                 ...
                 "Это твой престиж и статус в клане — виден в профиле:\n"]
        for need, name, desc in CLAN_RANKS:
            lines.append(f"<b>{name}</b> — от {need} LP · <i>{desc}</i>")
        lines.append("\nПросто будь активен — LP копятся, звание растёт. Проверь: <code>.p</code></blockquote>")
        return "\n".join(lines)
    if key == "rubles":
        return (f"{pe('crown')} <b>За рубли</b>\n<blockquote expandable>"
                f"<b>Brawl Pass, Brawl Pass Plus, Pro Pass и гемы</b> — покупаются за рубли по курсу. "
                f"За LP пассы больше не продаются.\n"
                f"Цены и оформление — у менеджера {STORE_MANAGER}. Оплата своими средствами, "
                f"выдача — через менеджера.</blockquote>")
    if key == "rules":
        return (f"{pe('scales')} <b>Правила STORE</b>\n<blockquote expandable>"
                ...
                f"{pe('contest')} «Право провести ивент» за LP даёт запустить один клановый ивент x2 LP.\n"
...
async def norms_info(update, context):
    await update.message.reply_text(
        f"{pe('excl')} <b>НОРМЫ КЛАНА {CLAN_NAME}</b>\n{CLAN_LINE}\n"
        f"<i>Регламент действует для каждого участника без исключений.</i>\n\n"

        f"{pe('star')} <b>1. Что нужно выполнять</b>\n"
        f"<blockquote>{pe('cup')} <b>Кубки:</b> минимум <b>+{WEEKLY_TROPHY_NORM}</b> за неделю\n"
        f"{pe('msg')} <b>Активность:</b> минимум <b>{WEEKLY_MSG_NORM}</b> сообщений за неделю\n"
        f"{pe('crown')} <b>Ранг:</b> достичь <b>Легендарного</b> за сезон\n"
        f"{pe('calendar')} <b>Присутствие:</b> не пропадать дольше <b>3 дней</b> без предупреждения</blockquote>\n"

        f"{pe('lp')} <b>2. Награда за нормы</b>\n"
        f"<blockquote>Нормы — это не только обязанность, но и <b>доход</b>:\n"
        f"• за каждую закрытую неделю кубков — <b>+{NORM_MONTH_PER_WEEK} LP</b>\n"
        f"• за каждую закрытую неделю активности — <b>+{NORM_MSG_WEEK_LP} LP</b>\n"
        f"Выплата приходит <b>1-го числа</b> за все закрытые недели месяца. "
        f"Закрывай нормы — растёт баланс и клановое звание.</blockquote>\n"

        f"{pe('calendar')} <b>3. Когда считаются</b>\n"
        f"<blockquote>Нормы кубков и активности <b>уже действуют</b>: неделя идёт Пн→Вс, проверка в воскресенье. "
        f"На старте — мягкий период без штрафов, чтобы все вошли в ритм.</blockquote>\n"

        f"{pe('shield')} <b>4. Если не выполнить</b>\n"
        f"<blockquote>За каждую невыполненную норму — <b>−{NORM_FAIL_PENALTY} LP</b> и норм-варн "
        f"(не больше одного в неделю). <b>3 норм-варна за месяц — исключение.</b>\n"
        f"Уезжаешь / экзамены — попроси админа поставить <b>отпуск</b>, и нормы встанут на паузу.</blockquote>\n"

        f"{pe('fire')} <b>Нормы — обязательное условие нахождения в клане.</b> "
        f"Не тянешь — освобождаешь место тем, кто готов играть и расти.",
        parse_mode=ParseMode.HTML)
...
_DOCS_VER_NAMES = {"6": "Империя", "5": "Цитадель", "4": "Адмирал", "3": "Флагман"}
...
async def on_quiz_callback(update, context):
    """6.0 Викторина 2.0: ответ кнопкой. Жать может ТОЛЬКО тот, кто запустил викторину."""
    cq = update.callback_query
    mid = getattr(cq.message, "message_id", None)
    st = _quiz_state.get(mid)
    if not st:
        return await cq.answer("Эта викторина уже завершена 🙂", show_alert=True)
    # только запустивший может отвечать
    if cq.from_user.id != st["starter"]:
        return await cq.answer("Это не твоя викторина — запусти свою: .quiz", show_alert=True)
    # таймаут
    if (time.time() - st["ts"]) > QUIZ_TIMEOUT_SEC:
        _quiz_state.pop(mid, None)
        right = st["options"][st["answer"] - 1]
        try:
            await cq.edit_message_text(
                f"{pe('clock')} <b>Время вышло!</b>\n{CLAN_LINE}\nПравильный ответ: <b>{esc(right)}</b>.",
                parse_mode=ParseMode.HTML)
        except Exception:
            pass
        return await cq.answer("Время вышло")
    try:
        pick = int((cq.data or "qz:0").split(":")[1])
    except Exception:
        return await cq.answer()
    ...
    letters = ["А", "Б", "В", "Г", "Д"]
    right_idx = st["answer"]
    right = st["options"][right_idx - 1]
    if pick == right_idx:
        ensure_user(cid, me)
        new_title = None
        if me.id not in EXCLUDED_IDS and me.id != LEADER_ID:
            conn.execute("UPDATE users SET quiz_wins=COALESCE(quiz_wins,0)+1 WHERE chat_id=? AND user_id=?", (cid, me.id))
            conn.commit()
            new_title = refresh_earned_title(cid, me.id)
        wins = quiz_wins_total(cid, me.id)
        tail = (f"\n{pe('star')} Новый титул: <b>{new_title}</b>!" if new_title else "")
        try:
            await cq.edit_message_text(
                f"{pe('cup')} <b>ВЕРНО!</b> · {st.get('catline','')}\n{CLAN_LINE}\n"
                f"<b>{esc(me.full_name)}</b> отвечает правильно: <b>{letters[right_idx-1]}. {esc(right)}</b> {pe('fire')}\n"
                f"{pe('game')} Побед в викторине: <b>{wins}</b>{tail}",
                parse_mode=ParseMode.HTML)
        except Exception as e:
            log_error("викторина верно", e)
        await cq.answer("Верно! 🎉")
    else:
        chosen = st["options"][pick - 1] if 1 <= pick <= len(st["options"]) else "?"
        try:
            await cq.edit_message_text(
                f"{pe('cross')} <b>НЕВЕРНО</b> · {st.get('catline','')}\n{CLAN_LINE}\n"
                f"<b>{esc(me.full_name)}</b> выбрал: <s>{esc(chosen)}</s>\n"
                f"{pe('check')} Правильный ответ: <b>{letters[right_idx-1]}. {esc(right)}</b>",
                parse_mode=ParseMode.HTML)
        except Exception as e:
            log_error("викторина неверно", e)
...
def _faq_text():
    ...
    return (
        ...
        f"<blockquote>Чтобы пользоваться ботом и STORE клана {CLAN_NAME}, нужно <b>принять это соглашение кнопкой</b>. "
        f"Нажимая «Принять», ты подтверждаешь, что прочитал и согласен со всеми пунктами. "
        f"Не согласен хотя бы с одним — нажми «Отклонить» и не пользуйся ботом.</blockquote>\n"
...
async def summon_all(update, context, text):
    """Сбор клана: красиво оформленный призыв + пинг всех участников. Только админ."""
    chat_id = update.effective_chat.id
    note = text.split(maxsplit=1)[1].strip() if len(text.split()) > 1 else ""
    rows = conn.execute("SELECT user_id, name FROM users WHERE chat_id=? AND COALESCE(gone,0)=0", (chat_id,)).fetchall()
    if not rows:
    ...
    try:
        ...
        pass
    # красивая шапка-карточка
    header = (
        f"{pe('mega')} <b>СБОР КЛАНА {CLAN_NAME}</b>\n"
        f"{CLAN_LINE}\n"
        f"{pe('fire')} <b>Все на связь, бойцы!</b>\n")
    if note:
        header += f"<blockquote>{pe('note')} {esc(note)}</blockquote>\n"
    else:
        header += f"<blockquote>Залетай в чат — клан собирается! Активность решает 💪</blockquote>\n"
    header += f"{pe('person')} Откликнулись ниже — не подведи команду 👇"
...
async def on_action_callback(update, context):
    ...
    if data.startswith("faq:"):
        ...
        if choice == "yes":
            ...
            try:
                await cq.edit_message_text(
                    f"{pe('check')} <b>Соглашение принято!</b> Бот тебе полностью доступен. Добро пожаловать в {CLAN_NAME}! {pe('fire')}\n"
                    f"{CLAN_LINE}\n"
                    f"{pe('warn')} <b>ВАЖНЫЙ ШАГ: привяжи свой игровой тег</b>\n"
                    f"<blockquote>Это нужно, чтобы бот видел твои кубки и ранг для норм и наград.</blockquote>\n\n"
                    f"{pe('person')} <b>Где взять тег?</b>\n"
                    f"<blockquote>1. Открой <b>Brawl Stars</b>.\n"
                    f"2. Нажми на свой <b>профиль</b> (аватарка вверху слева).\n"
                    f"3. Под твоим именем будет тег вида <b>#ABC123XY</b> (с решёткой).\n"
                    f"4. Нажми на тег — он скопируется.</blockquote>\n"
                    f"{pe('check')} <b>Как привязать?</b>\n"
                    f"<blockquote>Напиши боту прямо здесь или в чате:\n"
                    f"<code>.tag #ТВОЙ_ТЕГ</code>\n"
                    f"Например: <code>.tag #2PP0LV8GR</code></blockquote>\n"
                    f"{pe('cross')} <b>Частые ошибки:</b>\n"
                    f"<blockquote>• Это тег <b>из игры Brawl Stars</b>, а НЕ твой ник в Telegram.\n"
                    f"• Не путай букву «O» и ноль «0» — в тегах бывает только цифра 0.\n"
                    f"• Тег начинается с решётки <b>#</b>.</blockquote>\n"
                    f"{pe('bulb')} Готово? Проверь профиль: <code>.p</code> — там появятся твои кубки и ранг.",
                    parse_mode=ParseMode.HTML)
            except Exception:
                try:
    ...
    if data.startswith("buy:"):
        ...
        if _k in ("event", "title", "vip", "normpass", "quizboost"):
            ...
            if _k == "event":
                ...
                conn.execute("UPDATE users SET event_tickets=COALESCE(event_tickets,0)+1 WHERE chat_id=? AND user_id=?",
                             (chat_id, user.id))
                conn.commit()
                await cq.answer("Куплено! Билет на ивент твой 🎉", show_alert=True)
...
async def owner_command(update, context, first, parts):
    ...
    if first in ("сверка", "состав", "привязки", "roster", "tags"):
        ...
        chat_rows = conn.execute("SELECT user_id, name, username, brawl_tag FROM users WHERE chat_id=? AND COALESCE(gone,0)=0", (cid,)).fetchall()
        bound = {}
        unbound = []
        for r in chat_rows:
            if r["user_id"] in EXCLUDED_IDS:
                continue
            tg = (r["brawl_tag"] or "").upper().lstrip("#")
            if tg:
                bound[tg] = r
            else:
                unbound.append(r)
        clan_tags = {m["tag"] for m in members}
        # кто в клане, но не найден в чате (нет привязки к чату)
        in_clan_no_chat = [m for m in members if m["tag"] not in bound]
        # кто в чате привязал тег, но его НЕТ в клане (вышел/другой клан)
        bound_not_in_clan = [r for tg, r in bound.items() if tg not in clan_tags]

        def mention(r):
            nm = esc(r["name"] or "—")
            return f"<a href=\"tg://user?id={r['user_id']}\">{nm}</a>"

        lines = [f"{pe('stats')} <b>СВЕРКА СОСТАВА — {CLAN_NAME}</b>\n{CLAN_LINE}",
                 f"В клане по API: <b>{len(members)}</b> · в чате с привязкой: <b>{len(bound)}</b>\n"]
        if unbound:
            lines.append(f"{pe('warn')} <b>В чате, но НЕ привязали тег ({len(unbound)}):</b>")
            lines.append(" ".join(mention(r) for r in unbound[:40]))
            lines.append("")
        if in_clan_no_chat:
            lines.append(f"{pe('note')} <b>В клане, но не найдены в чате ({len(in_clan_no_chat)}):</b>")
            lines.append(", ".join(esc(m["name"]) for m in in_clan_no_chat[:40]))
            lines.append("")
        if bound_not_in_clan:
            lines.append(f"{pe('cross')} <b>Привязали тег, но НЕ в клане ({len(bound_not_in_clan)}):</b>")
            lines.append(" ".join(mention(r) for r in bound_not_in_clan[:40]))
        if not unbound and not in_clan_no_chat and not bound_not_in_clan:
            lines.append(f"{pe('check')} Всё сходится — все привязали теги и состоят в клане!")
    ...
    if first in ("розыгрыш", "giveaway", "раздача", "draw"):
        ...
        winners_n = nums[0] if len(nums) > 0 else 1
        prize = nums[1] if len(nums) > 1 else 100
        days = nums[2] if len(nums) > 2 else 7
        winners_n = max(1, min(20, winners_n))
        prize = max(1, min(LP_MAX, prize))
        days = max(1, min(30, days))
        pool = active_members(ALLOWED_CHAT_ID, days=days)
        if not pool:
            await update.message.reply_text(
                f"{pe('note')} Нет активных участников за {days} дн. для розыгрыша. "
                f"Формат: <code>.розыгрыш 2 150 7</code> — 2 победителя по 150 LP среди активных за 7 дней.",
                parse_mode=ParseMode.HTML)
            return True
        ...
        win_lines = "\n".join(
            f"{pe('gold')} <a href=\"tg://user?id={w['user_id']}\">{esc(w['name'])}</a> — <b>+{prize} LP</b>"
            for w in winners)
        try:
            await context.bot.send_message(
                ALLOWED_CHAT_ID,
                f"{pe('contest')} <b>РОЗЫГРЫШ LP — {CLAN_NAME}</b>\n{CLAN_LINE}\n"
                f"{pe('fire')} Среди <b>{len(pool)}</b> активных за {days} дн. бот разыграл призы!\n\n"
                f"{pe('cup')} <b>Победители:</b>\n<blockquote>{win_lines}</blockquote>\n"
                f"{pe('star')} Хочешь в следующий — будь активен в чате, и удача найдёт тебя! 🍀",
                parse_mode=ParseMode.HTML, disable_web_page_preview=True)
        except Exception as e:
            log_error("розыгрыш", e)
        await update.message.reply_text(
            f"{pe('check')} Розыгрыш проведён: <b>{len(winners)}</b> победит. по <b>{prize} LP</b> "
            f"(из {len(pool)} активных за {days} дн.). Объявление — в чате.", parse_mode=ParseMode.HTML)
        return True
...
class _DMMessage:
    """6.0: reply_text шлёт ответ в ЛС указанному пользователю (скрытные админ-команды)."""
    __slots__ = ("_real", "_bot", "_dm_id")

    def __init__(self, real_message, bot, dm_id):
    ...
    async def reply_text(self, text, **kwargs):
        ...
        try:
            return await self._bot.send_message(chat_id=self._dm_id, text=text, **kwargs)
        except Exception:
            return None
...
class _DMUpdate:
    """6.0: обёртка — ответы команды уходят в ЛС владельцу, а не в чат (скрытность LP/пранков)."""
    __slots__ = ("_real", "_msg")

    def __init__(self, real_update, bot, dm_id):
        object.__setattr__(self, "_real", real_update)
        object.__setattr__(self, "_msg", _DMMessage(real_update.message, bot, dm_id))
...
async def handle_triggers(update, context, text, low, parts):
    ...
    if (first in STAFF_AUTODELETE_COMMANDS and update.effective_chat
        ...
        try:
            ...
            pass
        # 6.0: скрытные команды (LP, реп, пранк, задать…) → ответ В ЛС владельцу, а не в чат.
        if first in STAFF_DM_FEEDBACK_COMMANDS:
            update = _DMUpdate(update, context.bot, update.effective_user.id)
        else:
    ...
    # 6.0: команды участников работают ТОЛЬКО в топике команд (и в ЛС). В остальных топиках/чате —
    # одно упоминание на человека, дальше молча игнорируем (чтобы не засорять чат).
    if (u and first in GATED_COMMANDS and COMMANDS_TOPIC_ID
            and update.effective_chat and update.effective_chat.type in ("group", "supergroup")):
        thread = getattr(update.message, "message_thread_id", None) if update.message else None
        is_staff = await can_use_admin(context, u.id)
        if thread != COMMANDS_TOPIC_ID and not is_staff:
            # удаляем команду из основного чата (чтобы не мусорила)
            try:
                await update.message.delete()
            except Exception:
                pass
            if u.id not in _cmd_topic_notified:
                _cmd_topic_notified.add(u.id)
                try:
                    await context.bot.send_message(
                        update.effective_chat.id,
                        f"{pe('note')} <b>{esc(u.full_name)}</b>, команды бота работают в отдельном топике "
                        f"«Команды» — пиши их там 🙏 Здесь общаемся, там играем.",
                        parse_mode=ParseMode.HTML, message_thread_id=COMMANDS_TOPIC_ID)
                except Exception:
                    pass
            return True
    ...
    if first in ("ивент", "event", "ивенты", "events"):
        ...
        # запуск/стоп — владелец всегда; игрок может запустить ОДИН ивент x2, если купил билет
        if sub in EVENT_TYPES or sub in ("стоп", "stop", "off"):
            me_id = update.effective_user.id
            owner = is_owner(me_id)
            tickets = 0
            if not owner:
                trow = conn.execute("SELECT COALESCE(event_tickets,0) AS t FROM users WHERE chat_id=? AND user_id=?",
                                    (ALLOWED_CHAT_ID, me_id)).fetchone()
                tickets = trow["t"] if trow else 0
            if not owner and tickets <= 0:
                # нет прав и нет билета — просто показываем статус (без спама)
            ...
            if sub in ("стоп", "stop", "off"):
                ...
                if ALLOWED_CHAT_ID:
                    try:
                        ...
                        pass
                return True
            label, mult, kind = EVENT_TYPES[sub]
            hrs = next((int(p) for p in parts[2:] if p.isdigit()), 24)
            hrs = max(1, min(EVENT_MAX_HOURS, hrs))
            launcher_note = ""
            if not owner:
                # игрок тратит билет: запускает «Час репутации» на 6 ч (соц-движ, не по кругу с LP)
                label, mult, kind, hrs = EVENT_TYPES["rep"][0], EVENT_TYPES["rep"][1], "rep", 6
                conn.execute("UPDATE users SET event_tickets=COALESCE(event_tickets,0)-1 WHERE chat_id=? AND user_id=?",
                             (ALLOWED_CHAT_ID, me_id))
                conn.commit()
                launcher_note = f"\n{pe('person')} Ивент запустил: <b>{esc(update.effective_user.full_name)}</b>"
            ...
            meta_set("event_kind", kind)
            # эффект для анонса
            if kind == "rep":
                effect = f"{pe('star')} Респекты дают <b>x{mult:g}</b> репутации — хвалите друг друга!"
            elif kind == "quiz":
                effect = f"{pe('game')} Викторина <b>без задержки</b> — играйте сколько хотите!"
            else:
                effect = f"{pe('lp')} LP за сообщения — <b>x{mult:g}</b>! Пишите, общайтесь, зарабатывайте больше!"
            await update.message.reply_text(
                f"{pe('check')} Запущен ивент <b>{label}</b> на <b>{hrs} ч</b>.", parse_mode=ParseMode.HTML)
            if ALLOWED_CHAT_ID:
                try:
                    await context.bot.send_message(ALLOWED_CHAT_ID,
                        f"{pe('mega')} <b>ИВЕНТ: {label}!</b>\n{CLAN_LINE}\n"
                        f"{effect}\n"
                        f"{pe('clock')} Действует <b>{hrs} ч</b>! {pe('fire')}{launcher_note}", parse_mode=ParseMode.HTML)
                except Exception:
                    pass
            return True
        # статус (всем)
        if is_owner(update.effective_user.id) and sub in ("", "help", "помощь"):
            await update.message.reply_text(
                ...
                f"<code>.event rep 6</code> — 💛 час репутации (x2 респекты)\n"
                f"<code>.event quiz 6</code> — 🧠 викторина-марафон (без задержки)\n"
    ...
    if first in ("ачивки", "achievements", "значки", "достижения"):
        ...
        for code, emoji, title, descr in ACHIEVEMENTS:
            if code in have:
                unlocked.append(f"{emoji} <b>{title}</b>")
            else:
                locked.append(f"🔒 <b>{title}</b> <i>— {descr}</i>")
        got = len(have)
        bar = progress_bar(got, len(ACHIEVEMENTS))
        blocks = [f"{pe('cup')} <b>ЗНАЧКИ — {esc(t.full_name)}</b>\n{CLAN_LINE}",
                  f"{pe('star')} Открыто: <b>{got}/{len(ACHIEVEMENTS)}</b>\n{bar}"]
        if unlocked:
            blocks.append(f"\n{pe('check')} <b>Получены:</b>\n<blockquote>" + "\n".join(unlocked) + "</blockquote>")
        if locked:
            blocks.append(f"{pe('lock')} <b>Ещё впереди:</b>\n<blockquote expandable>" + "\n".join(locked) + "</blockquote>")
    ...
    if first in ("викторина", "quiz", "квиз"):
        ...
        cat = q.get("c", "Brawl Stars")
        catline = quiz_catline(q.get("d", 2), cat)   # 6.0: «🟢 Лёгкий · Бойцы»
        # кнопки: по одной на вариант (callback: qz:<answer_index>)
        letters = ["А", "Б", "В", "Г", "Д"]
        kb_rows = []
        for i, o in enumerate(q["options"], 1):
            kb_rows.append([InlineKeyboardButton(f"{letters[i-1]}. {o}"[:60], callback_data=f"qz:{i}")])
        opts_txt = "\n".join(f"<b>{letters[i-1]}.</b> {esc(o)}" for i, o in enumerate(q["options"], 1))
        sent = await update.message.reply_text(
            f"{pe('game')} <b>ВИКТОРИНА BRAWL STARS</b>\n{CLAN_LINE}\n"
            f"{pe('folder')} <b>Категория вопроса:</b> {catline}\n\n"
            f"<b>{esc(q['q'])}</b>\n\n{opts_txt}\n\n"
            f"{pe('person')} Отвечает: <b>{esc(me.full_name)}</b> · жми кнопку 👇\n"
            f"{pe('warn')} <b>Одна попытка!</b> · {pe('star')} награда — престиж к титулу · {QUIZ_TIMEOUT_SEC} сек",
            parse_mode=ParseMode.HTML, reply_markup=InlineKeyboardMarkup(kb_rows))
        mid = getattr(sent, "message_id", None)
        if mid:
            _quiz_state[mid] = {"answer": q["answer"], "ts": time.time(), "options": q["options"],
                                "starter": me.id, "qid": qid, "cat": cat, "catline": catline}
        return True

    if first in ("ответ", "answer", "otvet"):
        await update.message.reply_text(
            f"{pe('note')} Викторина 2.0 теперь с кнопками — просто жми вариант под вопросом! "
            f"Запусти новую: <code>.quiz</code>", parse_mode=ParseMode.HTML)
        return True
    ...
    if first in ("титул", "title"):
        ...
        sub = parts[1].lower() if len(parts) > 1 else ""
        me = update.effective_user
        if sub in ("снять", "remove", "clear", "удалить", "delete"):
            # 6.0: если указана цель (@ник/ответ) и есть права — снимаем чужой титул
            tgt = _owner_target(update, parts)
            if tgt and tgt.id != me.id:
                if not (is_owner(me.id) or await can_use_admin(context, me.id)):
                    await update.message.reply_text(f"{pe('lock')} Снимать чужие титулы может только администрация.", parse_mode=ParseMode.HTML)
                    return True
                ensure_user(cid, tgt)
                conn.execute("UPDATE users SET title_text=NULL, title_until=NULL WHERE chat_id=? AND user_id=?", (cid, tgt.id))
                conn.commit()
                await update.message.reply_text(f"{pe('check')} Кастомный титул <b>{esc(tgt.full_name)}</b> снят.", parse_mode=ParseMode.HTML)
                return True
        ...
        if sub in ("список", "list", "витрина", "все", "all"):
            ...
            # 6.0: сезонные титулы-чемпиона (за топ месяца)
            lines.append(f"\n{pe('crown')} Сезонные регалии <i>(за топ месяца)</i>:")
            lines.append(f"🏆 <b>Чемпион сезона</b> — 1 место · 🥈🥉 <b>Призёр сезона</b> — 2-3 место (на месяц)")
            # следующая ближайшая цель
            nxt = None
            for need, title in QUEST_TITLES:
                if total < need:
                    nxt = f"квесты: ещё {need - total} до «{title}»"; break
            for need, title in QUIZ_TITLES:
                if qwins < need:
                    cand = f"викторина: ещё {need - qwins} до «{title}»"
                    nxt = cand if not nxt else nxt; break
            lines.append(f"\n{pe('person')} Твой титул сейчас: <b>{esc(cur_show) if cur_show else 'нет — выполни 3 квеста или выиграй 3 викторины'}</b>")
            if nxt:
                lines.append(f"{pe('star')} Ближайшая цель — {nxt}")
        ...
        if sub in ("купить", "buy"):
            ...
            if me.id in EXCLUDED_IDS:
                ...
                return True
            if title_has_violation(wanted):
                await update.message.reply_text(
                    f"{pe('warn')} Такой титул нарушает кодекс клана. Придумай приличный — без оскорблений и мата.",
                    parse_mode=ParseMode.HTML)
                return True
            ensure_user(cid, me)
            bal = (get_user(cid, me.id) or {"lp": 0})["lp"]
        ...
        if sub in ("дать", "give", "set"):
            ...
            if not wanted:
                ...
                return True
            if title_has_violation(wanted):
                await update.message.reply_text(f"{pe('warn')} Этот титул нарушает кодекс — выбери приличный.", parse_mode=ParseMode.HTML)
                return True
            ensure_user(cid, tgt)
...
_quiz_state = {}     # message_id -> {"answer":int(1-based), "ts":float, "options":[...], "starter":int, "qid":int}


def _quiz_seen_set(chat_id, user_id):
    """6.0: индексы вопросов, которые игрок уже видел (хранятся в users.quiz_seen как CSV)."""
    try:
        row = conn.execute("SELECT quiz_seen FROM users WHERE chat_id=? AND user_id=?", (chat_id, user_id)).fetchone()
        raw = (row["quiz_seen"] if row and "quiz_seen" in row.keys() else "") or ""
        return set(int(x) for x in raw.split(",") if x.strip().isdigit())
    except Exception:
        return set()


def _quiz_mark_seen(chat_id, user_id, qid):
    """6.0: помечает вопрос показанным. Когда показаны все — сбрасываем и начинаем новый круг."""
    seen = _quiz_seen_set(chat_id, user_id)
    seen.add(qid)
    if len(seen) >= len(QUIZ_QUESTIONS):
        seen = {qid}   # прошли весь пул — начинаем заново (но не повторяем последний)
    conn.execute("UPDATE users SET quiz_seen=? WHERE chat_id=? AND user_id=?",
                 (",".join(str(x) for x in sorted(seen)), chat_id, user_id))
    conn.commit()


def _quiz_pick(chat_id, user_id):
    """6.0: выбирает случайный вопрос, которого игрок ещё НЕ видел. Возвращает (qid, question)."""
    seen = _quiz_seen_set(chat_id, user_id)
    pool = [i for i in range(len(QUIZ_QUESTIONS)) if i not in seen]
    if not pool:
        pool = list(range(len(QUIZ_QUESTIONS)))   # всё видел — берём из полного пула
    qid = random.choice(pool)
    return qid, QUIZ_QUESTIONS[qid]
...
QUIZ_QUESTIONS = [
    # d: 1=Лёгкий 2=Средний 3=Сложный 4=Эксперт
    # --- Бойцы ---
    {"c": "Бойцы", "d": 1, "q": "Какой боец достаётся новому игроку самым первым?", "options": ["Кольт", "Шелли", "Эль Примо", "Ник"], "answer": 2},
    {"c": "Бойцы", "d": 1, "q": "К какому классу относится Эль Примо?", "options": ["Танк", "Снайпер", "Поддержка", "Артиллерия"], "answer": 1},
    {"c": "Бойцы", "d": 1, "q": "Какой боец лечит союзников своей атакой?", "options": ["Кольт", "Поко", "Бык", "Шелли"], "answer": 2},
    {"c": "Бойцы", "d": 2, "q": "Кто из этих бойцов — легендарный (Legendary)?", "options": ["Спайк", "Кольт", "Джесси", "Нита"], "answer": 1},
    {"c": "Бойцы", "d": 2, "q": "Какой супер у Леона?", "options": ["Невидимость", "Лечение", "Щит", "Прыжок"], "answer": 1},
    {"c": "Бойцы", "d": 2, "q": "Какой боец атакует в ближнем бою (мили)?", "options": ["Мортис", "Бо", "Бролок", "Пайпер"], "answer": 1},
    {"c": "Бойцы", "d": 3, "q": "Чем атакует Барли?", "options": ["Бутылками с зажигательной смесью", "Стрелами", "Кулаками", "Ракетами наведения"], "answer": 1},
    {"c": "Бойцы", "d": 2, "q": "Какой эффект накладывает атака Кроу?", "options": ["Яд", "Заморозка", "Поджог", "Оглушение"], "answer": 1},
    {"c": "Бойцы", "d": 3, "q": "Кто из этих бойцов — снайпер с большим уроном на дистанции?", "options": ["Пайпер", "Эль Примо", "Поко", "Джин"], "answer": 1},
    {"c": "Бойцы", "d": 2, "q": "Какой боец считается классическим танком с большим запасом здоровья?", "options": ["Бык", "Кольт", "Брок", "Тик"], "answer": 1},
    {"c": "Бойцы", "d": 2, "q": "Чем в основном атакует Дайнамайк?", "options": ["Бросает динамит", "Стреляет лазером", "Кидает ножи", "Лечит"], "answer": 1},
    {"c": "Бойцы", "d": 3, "q": "Какой боец призывает турель своим супером?", "options": ["Джесси", "Шелли", "Нита", "Шторм"], "answer": 1},
    {"c": "Бойцы", "d": 3, "q": "Кто призывает медведя своим супером?", "options": ["Нита", "Джесси", "Пэм", "Роза"], "answer": 1},
    {"c": "Бойцы", "d": 4, "q": "Сколько у каждого бойца уникальных Звёзд силы (Star Power) на выбор?", "options": ["1", "2", "3", "4"], "answer": 2},
    {"c": "Бойцы", "d": 4, "q": "Сколько у каждого бойца уникальных гаджетов на выбор?", "options": ["1", "2", "3", "4"], "answer": 2},
    {"c": "Бойцы", "d": 3, "q": "Какой боец оглушает врагов в зоне своим супером (удар по земле)?", "options": ["Фрэнк", "Поко", "Джесси", "Кольт"], "answer": 1},
    # --- Режимы ---
    {"c": "Режимы", "d": 1, "q": "Сколько кристаллов нужно собрать и удержать в режиме «Захват кристаллов»?", "options": ["7", "10", "12", "5"], "answer": 2},
    {"c": "Режимы", "d": 1, "q": "Сколько голов нужно для победы в «Броулбол»?", "options": ["1", "2", "3", "5"], "answer": 2},
    {"c": "Режимы", "d": 2, "q": "Сколько игроков в одиночном режиме «Выживание» (Showdown)?", "options": ["8", "10", "6", "12"], "answer": 2},
    {"c": "Режимы", "d": 1, "q": "Сколько игроков в каждой команде в большинстве режимов 3 на 3?", "options": ["2", "3", "4", "5"], "answer": 2},
    {"c": "Режимы", "d": 2, "q": "В каком режиме нужно защищать сейф своей команды и ломать чужой?", "options": ["Ограбление (Heist)", "Нокаут", "Броулбол", "Награда"], "answer": 1},
    {"c": "Режимы", "d": 2, "q": "Как называется режим, где команда без возрождений — кто всех выбьет?", "options": ["Нокаут", "Захват кристаллов", "Броулбол", "Ограбление"], "answer": 1},
    {"c": "Режимы", "d": 2, "q": "В режиме «Награда за поимку» (Bounty) очки-звёзды дают за что?", "options": ["Устранение соперников", "Сбор кристаллов", "Голы", "Удержание зоны"], "answer": 1},
    {"c": "Режимы", "d": 3, "q": "В каком режиме нужно удерживать особую зону дольше соперника?", "options": ["Горячая зона (Hot Zone)", "Ограбление", "Броулбол", "Выживание"], "answer": 1},
    {"c": "Режимы", "d": 3, "q": "Сколько по умолчанию команд в дуо-режиме «Выживание»?", "options": ["5 пар", "4 пары", "10 пар", "3 пары"], "answer": 1},
    # --- Механика ---
    {"c": "Механика", "d": 2, "q": "От чего заряжается супер-способность бойца?", "options": ["От нанесённого урона", "От времени", "Сразу бесплатно", "От ходьбы"], "answer": 1},
    {"c": "Механика", "d": 3, "q": "На каком уровне у бойца открывается «Звезда силы» (Star Power)?", "options": ["7", "9", "11", "5"], "answer": 2},
    {"c": "Механика", "d": 3, "q": "На каком уровне открывается гаджет (Gadget)?", "options": ["7", "9", "11", "1"], "answer": 1},
    {"c": "Механика", "d": 2, "q": "Какой максимальный уровень прокачки у бойца?", "options": ["9", "10", "11", "15"], "answer": 3},
    {"c": "Механика", "d": 2, "q": "Что нужно, чтобы повышать уровень бойца?", "options": ["Очки силы и монеты", "Только гемы", "Кубки", "Билеты"], "answer": 1},
    {"c": "Механика", "d": 4, "q": "Сколько гаджетов можно взять в один бой?", "options": ["1", "2", "3", "Сколько угодно"], "answer": 1},
    {"c": "Механика", "d": 2, "q": "Что делают кусты на карте для бойца?", "options": ["Прячут от врагов", "Лечат", "Ускоряют", "Дают щит"], "answer": 1},
    {"c": "Механика", "d": 3, "q": "Что произойдёт со здоровьем бойца, если он выйдет из боя и не получает урон?", "options": ["Восстановится со временем", "Останется как есть", "Будет падать", "Обнулится"], "answer": 1},
    # --- Валюта ---
    {"c": "Валюта", "d": 1, "q": "Какая валюта в игре — премиальная (покупается за деньги)?", "options": ["Гемы", "Монеты", "Очки силы", "Кубки"], "answer": 1},
    {"c": "Валюта", "d": 1, "q": "Что игрок получает за победы и что поднимает рейтинг бойца?", "options": ["Кубки (трофеи)", "Гемы", "Монеты", "Билеты"], "answer": 1},
    {"c": "Валюта", "d": 2, "q": "Что в основном тратится на улучшение бойцов вместе с очками силы?", "options": ["Монеты", "Гемы", "Кубки", "Звёзды"], "answer": 1},
    # --- Общее ---
    {"c": "Общее", "d": 1, "q": "Какая компания разработала Brawl Stars?", "options": ["Supercell", "Riot Games", "Epic Games", "Valve"], "answer": 1},
    {"c": "Общее", "d": 2, "q": "Как называется соревновательный режим с рангами и драфтом бойцов?", "options": ["Ранговый режим", "Клубная лига", "Чемпионат мира", "Турнир"], "answer": 1},
    {"c": "Общее", "d": 1, "q": "Как в игре называется объединение игроков, куда вступают по тегу?", "options": ["Клуб", "Гильдия", "Отряд", "Альянс"], "answer": 1},
    # --- Сложные/эксперт «вечные» факты ---
    {"c": "Общее", "d": 3, "q": "В каком году Brawl Stars вышел в мировом релизе (глобально)?", "options": ["2016", "2017", "2018", "2020"], "answer": 3},
    {"c": "Общее", "d": 3, "q": "Какую из этих игр НЕ выпускала студия Supercell?", "options": ["Clash of Clans", "Clash Royale", "PUBG Mobile", "Boom Beach"], "answer": 3},
    {"c": "Общее", "d": 2, "q": "Сколько максимум участников может быть в одном клубе?", "options": ["25", "30", "50", "100"], "answer": 2},
    {"c": "Режимы", "d": 3, "q": "До скольких выигранных раундов идёт матч в режиме «Нокаут»?", "options": ["1 раунд", "2 раунда (из 3)", "3 раунда (из 5)", "5 раундов"], "answer": 2},
    {"c": "Механика", "d": 4, "q": "Сколько Звёзд силы можно использовать в ОДНОМ бою?", "options": ["1", "2", "3", "Все сразу"], "answer": 1},
    {"c": "Бойцы", "d": 4, "q": "Какой класс в игре отвечает за стрельбу по навесной траектории через стены (Барли, Дайнамайк, Тик)?", "options": ["Артиллерия", "Снайпер", "Танк", "Поддержка"], "answer": 1},
]
...
_TITLE_BANNED = [
    "хуй", "хуя", "хуе", "хуё", "пизд", "ебан", "еба", "ебл", "ёб", "еби", "бляд", "блять",
    "сука", "сук", "мудак", "мудил", "гандон", "пидор", "пидар", "педик", "далбо", "долбо",
    "залуп", "очко", "анус", "хач", "нигер", "ниггер", "fuck", "shit", "bitch", "nigg", "cunt", "dick",
]


def title_has_violation(s):
    """6.0: True, если в титуле есть мат/нарушение кодекса."""
    low = (s or "").lower().replace(" ", "").replace(".", "").replace("_", "").replace("-", "")
    # частые замены символов
    for a, b in (("0", "о"), ("o", "о"), ("a", "а"), ("e", "е"), ("3", "е"), ("y", "у"), ("x", "х"), ("c", "с")):
        low = low.replace(a, b)
    return any(bad in low for bad in _TITLE_BANNED)
...
async def job_rank_scan(context):
    ...
    for r in rows:
        ...
        if ent:
            ...
            for tn, lp in awards:
                achievements.append(f"{esc(disp_name(r))} — <b>{tn}</b>! <b>+{lp} LP</b>")
            ranked_cnt += 1
    ...
    if achievements:
        try:
            await context.bot.send_message(
                ALLOWED_CHAT_ID,
                f"{pe('cup')} <b>РАНГОВЫЕ ДОСТИЖЕНИЯ МЕСЯЦА</b>\n{CLAN_LINE}\n" + "\n".join(achievements[:20]),
                parse_mode=ParseMode.HTML)
        except Exception as e:
...
async def counter(update, context):
    ...
    try:
        await flush_rankup_announcements(context)
    except Exception:
        pass
...
async def welcome(update, context):
    for member in update.message.new_chat_members:
        ...
        await update.message.reply_text(
            ...
            f"Общайся в чате — за сообщения капают {pe('lp')} LP. Также действует норма кубков (+1450/нед). "
...
def _help_section(key):
    ...
    if key == "profile":
        return (f"{pe('person')} <b>Профиль и статистика</b>\n<blockquote expandable>"
                ...
                "• <code>.quest</code> — задание дня (даёт титулы!) · <code>.achievements</code> — твои значки · "
                "<code>.quiz</code> — викторина по Brawl Stars (кнопки, одна попытка, титулы за победы)\n"
                "• <code>.title buy ТЕКСТ</code> — личный титул в профиле · <code>.title list</code> — витрина всех титулов\n"
    ...
    if key == "admin":
        return (f"{pe('shield')} <b>Админ-команды</b> <i>(работают только у администрации)</i>\n<blockquote expandable>"
                ...
                "<code>.poll Вопрос? | вар1 | вар2</code> · <code>.конкурс</code> (в личке)\n\n"
                "<b>Титулы:</b> <code>.title give @кто ТЕКСТ</code> — выдать · <code>.title clear @кто</code> — снять чужой титул "
                "<i>(мат в титулах блокируется автоматически)</i>\n\n"
                "<b>Чистка топов:</b> <code>.чистка</code> — авто-удалить вышедших из чата · "
                "<code>.удалить</code> (ответом/@/ID) — убрать из системы · "
                "<code>.отвязать</code> (ответом/@/ID) — снять чужой игровой тег\n\n"
                "<b>Ивенты:</b> <code>.event double 24</code> / <code>.event triple 12</code> / <code>.event happy 6</code> — запустить x2/x3 LP · "
                "<code>.event stop</code> — остановить\n\n"
                "<b>━━ Только владелец ━━</b>\n"
                "<b>Отчёты и аналитика:</b> <code>.отчет</code> (Excel за день в личку) · <code>.отчётнеделя</code> · "
                "<code>.отчётмесяц</code> <i>(большая сводка)</i> · <code>.analytics</code> · <code>.modlog</code> · "
                "<code>.актив</code> <i>(активность по дням)</i> · <code>.ошибки</code> <i>(журнал сбоев)</i>\n\n"
                ...
                "<code>.норма ранг</code> / <code>.норма кубки</code> (ответом) — зачесть норму\n\n"
                "<b>Резерв/бэкап:</b> <code>.backup</code> — выгрузить базу <i>(авто по понедельникам)</i>\n\n"
                "<b>Квесты/розыгрыши:</b> <code>.форсквест @кто</code> — завершить квест участнику · "
                "<code>.lottery draw</code> — провести лотерею\n\n"
                "<b>Ранги:</b> <code>.rank test [#ТЕГ]</code> — сырые значения рангов из боёв (сверка карты)\n\n"
                "<b>Точные значения / пранк:</b> <code>.задать реп/лп/кубки/ранг @кто N</code> — задать значение · "
                "<code>.пранк @кто поле значение</code> — фейк-профиль · <code>.пранкменю</code> (в личке) — меню пранков\n\n"
                "<b>Приветствия:</b> <code>.утро</code> / <code>.ночь</code> — показать сейчас "
                "<i>(бот сам шлёт и закрепляет в 8:00 и 22:00 МСК)</i> · "
                "<code>.добавитьгиф утро/ночь</code> (ответом на гиф) · <code>.гифы</code> · <code>.очиститьгифы утро/ночь</code>\n\n"
                "<b>Прочее:</b> <code>.id</code> — узнать chat_id · <code>.emojiid</code> (ответом на премиум-эмодзи)</blockquote>")
    return _help_intro()
...
def build_weekly_report(chat_id):
    ...
    name, mult, _kind = event_active()
    if name:
        lines.append(f"{pe('mega')} Активен ивент: {name}")
    return "\n".join(lines)
...
async def _send_greeting(context, chat_id, text, gif):
    ...
    Если гифка не отправилась (битая ссылка) — пробуем хотя бы текст. Возвращает message_id или None."""
    try:
        if gif:
            m = await context.bot.send_animation(chat_id, animation=gif, caption=text, parse_mode=ParseMode.HTML)
        else:
            m = await context.bot.send_message(chat_id, text=text, parse_mode=ParseMode.HTML)
...
async def _repin_greeting(context, new_msg_id, which):
    """6.0: закрепляет утреннее/ночное приветствие, откручивая предыдущее закреплённое приветствие.
    which = 'morning'|'night'. Хранит id закреплённых в meta. Тихо, без уведомления о закрепе."""
    if not new_msg_id or not ALLOWED_CHAT_ID:
        return
    # открепляем оба прошлых приветствия (утро+ночь), чтобы не копились
    for key in ("pinned_morning_id", "pinned_night_id"):
        old = meta_get(key)
        if old:
            try:
                await context.bot.unpin_chat_message(ALLOWED_CHAT_ID, int(old))
            except Exception:
                pass
            meta_set(key, "")
    # закрепляем новое (без уведомления участников)
    try:
        await context.bot.pin_chat_message(ALLOWED_CHAT_ID, new_msg_id, disable_notification=True)
        meta_set(f"pinned_{which}_id", str(new_msg_id))
    except Exception as e:
        log_error("закреп приветствия", e)


async def job_morning_greeting(context):
    if not GREETINGS_ENABLED:
        return
    mid = await _send_greeting(context, ALLOWED_CHAT_ID, build_morning_message(ALLOWED_CHAT_ID), await _morning_gif())
    await _repin_greeting(context, mid, "morning")   # 6.0: авто-закреп утреннего


async def job_night_greeting(context):
    if not GREETINGS_ENABLED:
        return
    mid = await _send_greeting(context, ALLOWED_CHAT_ID, build_night_message(), await _night_gif())
    await _repin_greeting(context, mid, "night")     # 6.0: авто-закреп ночного
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(on_quiz_callback, pattern="^qz:"))   # 6.0: викторина 2.0 кнопками
